document.addEventListener("alpine:init", () => {
  Alpine.data("orderPage", () => ({
    ...
    // ==========================
    pendingOrders: [],
    apiGetPendingOrders: "http://localhost:3000/api/admin-order/PENDING",
    apiSetOrderStatus: "http://localhost:3000/api/admin-order/set-status",
    allOrders: [],
    apiGetAllOrders: "http://localhost:3000/api/admin-order/get-all-order",
    apiGetOrderLog: "http://localhost:3000/api/admin-order/get-order-log",
    apiAddOrderLog: "http://localhost:3000/api/admin-order/add-order-log",
    apiSetOrderDone: "http://localhost:3000/api/admin-order/set-done",

    isOrderLogModalOpen: false,
    ...
    async fetchAllOrders() {
      ...
      try {
        const res = await fetch(this.apiGetAllOrders, {
          headers: { Authorization: `Bearer ${this.token}` },
        });
        const json = await res.json();
        this.allOrders = json.status ? json.data : [];
      } catch (err) {
        console.error("ERR FETCH ALL ORDERS:", err);
      }
    },

    async viewOrderLog(orderId) {
      this.isOrderLogModalOpen = true;
      this.currentOrderId = orderId;
      this.orderLogs = [];

      try {
        const res = await fetch(`${this.apiGetOrderLog}/${orderId}`, {
          headers: { Authorization: `Bearer ${this.token}` },
        });
        const json = await res.json();
        if (json.status) {
    ...
    openAddLogModal(orderId) {
      this.isAddLogModalOpen = true;
      this.currentOrderId = orderId;
      this.newLog = "";
    },

    async addOrderLog() {
      if (!this.newLog) return alert("Harap isi log");
      try {
        const res = await fetch(this.apiAddOrderLog, {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${this.token}` },
          body: JSON.stringify({ order_id: this.currentOrderId, log: this.newLog }),
        });
        const json = await res.json();
        if (json.status) {
          ...
          alert(json.message || "Gagal menambahkan log");
        }
      } catch (err) {
        console.error(err);
    ...
    async setOrderDone(orderId) {
      if (!confirm("Yakin ingin set order menjadi DONE?")) return;
      try {
        const res = await fetch(this.apiSetOrderDone, {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${this.token}` },
        ...
        const json = await res.json();
        if (json.status) {
          alert("Order berhasil diset DONE");
          window.location.reload();
        } else {
          alert(json.message || "Gagal set order DONE");
        }
      } catch (err) {
        console.error(err);
        alert("Terjadi kesalahan jaringan");
    ...
    async fetchPendingOrders() {
      ...
      try {
        const res = await fetch(this.apiGetPendingOrders, {
          headers: { Authorization: `Bearer ${this.token}` },
        });
        const json = await res.json();
        this.pendingOrders = json.status ? json.data : [];
      } catch (err) {
        console.error("ERR FETCH PENDING ORDERS:", err);
      }
    },

    async setStatus(transactionId, status) {
      if (!confirm(`Yakin ingin mengubah status menjadi "${status}"?`)) return;

      try {
        const res = await fetch(this.apiSetOrderStatus, {
          method: "POST",
          headers: {
    ...
    init() {
      this.checkAuth();
      this.fetchAllOrders();       // <-- fetch semua order
